...
class CrossNodeAttention(BertSelfAttention):
    def __init__(self, config):
        super(CrossNodeAttention, self).__init__(config)
        self.output_attentions = False
    ...
    def forward(self, hidden_states,   # B N L D
                        strcture_states,   # B N 1 D
                        attention_mask=None, rel_pos=None):
                        
        #concat the node_states to the hidden_states
        hidden_states_structured = torch.cat([hidden_states, strcture_states], dim=3)  # B N L 2D
        # concat the dimension N to the dimension L: B N L 2D -> B N*L 2D
        hidden_states_structured = hidden_states_structured.view(-1, hidden_states_structured.size(2), hidden_states_structured.size(3))  # B N*L 2D
        hidden_states = hidden_states.view(-1, hidden_states.size(2), hidden_states.size(3))  # B N*L D
...
class GraphFormersForNeighborPredict(GraphTuringNLRPreTrainedModel):
    ...
    def infer(self, input_ids_node_and_neighbors_batch, attention_mask_node_and_neighbors_batch,
        ...
        with record_function("infer_preparation"):
    ...
    def forward(self, input_ids_query_and_neighbors_batch, attention_mask_query_and_neighbors_batch,
                ...
                input_ids_key_and_neighbors_batch, attention_mask_key_and_neighbors_batch, mask_key_and_neighbors_batch,
                **kwargs):
        with record_function("query_embeddings"):
            query_embeddings = self.infer(input_ids_query_and_neighbors_batch, attention_mask_query_and_neighbors_batch,
                                        mask_query_and_neighbors_batch)
        with record_function("key_embeddings"):
            key_embeddings = self.infer(input_ids_key_and_neighbors_batch, attention_mask_key_and_neighbors_batch,
                                        mask_key_and_neighbors_batch)
        with record_function("score_loss"):
